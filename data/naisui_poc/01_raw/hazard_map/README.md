...
## 2026-09-16追記：公式浸水深・地盤高CSVを取得（未統合）

東京都オープンデータカタログ「浸水予想区域図（改定） 浸水深・地盤高」から、城南地区河川流域（品川区含む）の
浸水深・地盤高CSV（点群、緯度経度、CC BY 4.0）を取得し、`tokyo_opendata_jonankaisei/`に配置した。
想定降雨は本PoC採用シナリオ（時間最大153mm・総雨量690mm）と一致する。ただし、このCSVは2018年改定の「浸水予想区域図」（河川と下水道による浸水）に対応し、下記PDFは2026年公表の「雨水出水浸水想定区域図」（下水道による浸水）である。両者は同一図面ではない。
中延・二葉範囲での点密度も確認済み（詳細は`tokyo_opendata_jonankaisei/README.md`）。

**ただし座標系・浸水深の定義（最大値かどうか等）は未検証であり、QGISでの確認・変換前は評価に使用しないこと。**
CSVの検証・変換だけでは2026年版の面データ未入手という制約は解消せず、PDF派生ベクタを置き換えられない。
