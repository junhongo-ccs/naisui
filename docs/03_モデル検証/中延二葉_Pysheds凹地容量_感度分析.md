...
|---|---|---:|---|---:|---:|
| Case A | `median3_cap0p5m` | 0.5m | 軽微な道路冠水レベルに抑えた場合の挙動 | 約0.49m | 約68,497m³ |
| Case B | `median3_cap1m` | 1.0m | 床下・床上浸水境界レベルの標準的な挙動 | 約0.99m | 約77,013m³ |
| Case C | `median3_cap1p5m` | 1.5m | アンダーパス等の深い貯留を許容した場合の挙動 | 約1.49m | 約81,121m³ |

最終貯留量は2026-09-25に地上面積で再集計した値（旧値103,838／116,749／122,975m³は、EPSG:3857の名目画素面積による約1.52倍の過大。`docs/01_概要・計画/PLATEAU導入計画.md` 2.2節）。湛水深とケース間の大小関係は変わらない。
