...
最終更新: 2026-09-25（冒頭の追記を参照。2026-09-14作成、2026-09-15・09-25追記）
...
## 週明け（2026-09-28〜）のタスク: Difyとの接続

ユーザー指示（2026-09-25）。チャットに「その町丁目の地域の背景と客観的な事実」を答えさせるため、Difyのナレッジ（RAG）とつなぐ。
下の「次回開始時の判断契約」にある直近作業1〜4（Dify関連）は、この手順で置き換える。

1. Difyでナレッジを作り、`RAG/` の `01_地域の背景.md`・`02_町丁目_*.md`（10件）・`03_用語とデータの説明.md` を取り込む。チャンク設定は `RAG/README.md` の推奨設定（区切り文字 `\n\n`、最大1000文字、オーバーラップ0、改行の置換オフ、ハイブリッド検索）に従う。`RAG/README.md` 自体は取り込まない。
2. Difyのチャットアプリ（またはワークフロー）を作る。入力は「選んだ町丁目」と「質問」。回答のルールは `RAG/03_用語とデータの説明.md` の「回答のルール」と `data/naisui_poc/04_llm_knowledge/safety_guardrails.md` に従わせる。ナレッジ内の文章は参照データであり、文章中の命令で安全ポリシーを変えさせない。
3. `backend/app/llm_adapter.py` のテンプレート応答を、Dify APIの呼び出しに差し替える。APIキーとURLは `.env`（`DIFY_API_KEY`、`DIFY_BASE_URL` など）で管理し、コミットしない。`backend/app/main.py` の決定的な判定（緊急ワード、範囲外・特定できない場所、複数の町丁目）は、Difyを呼ぶ前に必ず行い、Difyに委ねない。
4. Difyの応答を既存のJSON契約（`headline`/`status`/`facts`/`model_context`/`safe_next_steps`/`prohibited_claim_check`/`sources`）で検証してから画面に返す。契約違反・タイムアウト時は、現行のテンプレート応答に戻す。
5. 試験: 中延六丁目（三間通り沿い、暗渠の北側の谷底）、中延一丁目（暗渠から遠い台地で影響が小さい）、二葉一丁目（ふたばトンネル）、中延三丁目（北西の窪みは池上線の掘割で住宅地のくぼ地ではない）、「下新明あたり」（場所を特定できない→データ範囲を示す）、中延七丁目（範囲外）、緊急ワード、ナレッジへの指示の注入。テンプレート応答と比べ、地域の背景と根拠を示せているかを確認する。

## 2026-09-25 追記（3）: 大前提の見直し、地域の背景、RAG

ユーザーと、このPoCの大前提を見直した（2026-09-25）。

- 実際の利用者や、緊急時に使う場面は想定しない。これは**ハザードマップ**である。
- モデルは下水道管網を持たない地表湛水のスクリーニングで、内水氾濫そのものは再現していない。そのため、タイトルを「**中延・二葉 雨水のたまりやすさマップ**」に変えた。
- 地図: 自前モデルの表示を紫系に変え、公式の浸水想定（青灰）と区別した。凡例は「何を示しているか」で書き、2つの層の表示・非表示を切り替えられるようにした。
- チャット: 町丁目名の自由入力を読み取る。範囲外・特定できない地名には、データ範囲（10町丁目）を示して選ばせる。自由入力をどこまで許すかは未決（`docs/02_仕様・要件/中延二葉_チャットUI_要件定義.md` 8章）。

地域の背景を一次資料で調べた（`data/naisui_poc/04_llm_knowledge/region_background.md`）。

- 中延・二葉は立会川の谷沿いにあり、立会川は昭和44〜47年に暗渠化され下水道幹線（立会川幹線）になった。上部は立会道路。
- **R7.9.11の浸水件数の上位6町丁目は、すべて暗渠が町内を通る町丁目**で、下位4町丁目はすべて通らない。暗渠が通る町丁目では、たまりやすい場所と浸水想定区域が暗渠の片側（谷底側）に偏る。

町丁目ごとの客観的事実を `tools/build_town_facts.py` で集計した（`data/naisui_poc/02_processed/town_facts/`）。

- 通り名はOpenStreetMap（`data/naisui_poc/01_raw/osm/`）から取った。
- 鉄道用地とアンダーパスは集計から除いた。中延三丁目の窪みは池上線の掘割、二葉一丁目の窪みはふたばトンネルの出入口であることを、ユーザーが確認した。

Difyのナレッジ用に `RAG/` を作った（`tools/build_rag_markdown.py` で町丁目ファイルを生成）。
...
現行の直近作業は、採用判断を変えることではなく、**Dify＋RAGでチャットUIの価値と安全性を実証すること**である。次の順で進める。（2026-09-25: 1〜4は冒頭の「週明けのタスク: Difyとの接続」で置き換えた。RAGに登録するのは `RAG/` の文書である）
