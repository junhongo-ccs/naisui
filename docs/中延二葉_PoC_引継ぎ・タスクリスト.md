...
本文書 5章「明日の優先タスクリスト」の「Pyshedsの流向・窪地処理と簡易湛水モデルの役割を比較し、最終採用モデルを決める」は決着した。詳細は `docs/中延二葉_モデル実績照合レビュー.md`。
...
- UIでのモデルと実績の扱いは「実績を主・モデルを補助」ではなく、**「モデルを主とし、実績との乖離を減らす校正・検証材料として実績を使う」**方針にユーザーが修正済み（`docs/中延二葉_チャットUI_要件定義.md`に反映済み）
...
判断が衝突した場合の優先順位は、`docs/中延二葉_チャットUI_要件定義.md` の現行UI方針、`docs/中延二葉_モデル実績照合レビュー.md` のモデル採用・検証判断、`docs/中延二葉_段階3_Dify_LLM安全ガードレール仕様.md` の安全制約、の順とする。
...
詳細なLLM安全要件は `docs/中延二葉_段階3_Dify_LLM安全ガードレール仕様.md`、PDF派生データの根拠は `docs/品川_ハザードPDF地理参照・色採取記録.md` を正とする。
...
| PoC全体像 | `docs/中延二葉_内水氾濫AI_GIS_PoC全体概要.md` |
| 簡易モデルの校正・評価方針 | `docs/中延二葉_簡易湛水モデル_校正評価仕様.md` |
| Pysheds感度分析 | `docs/中延二葉_Pysheds凹地容量_感度分析.md` |
| PDF派生ハザードの作業根拠 | `docs/品川_ハザードPDF地理参照・色採取記録.md` |
| LLM安全設計 | `docs/中延二葉_段階3_Dify_LLM安全ガードレール仕様.md` |
...
- [ ] `docs/UI_デザイン受け入れ基準.md` に沿って地図・凡例・データ時刻・モデルの校正状態をUIで明示する。
